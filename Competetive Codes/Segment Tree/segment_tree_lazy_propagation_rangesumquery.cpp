...
int updateRangeUtil(int rstart,int rend,int updateIndexStart,int updateIndexEnd,int diff,int index) {
    //here diff represents the value to be added
    ...
    if(updateIndexStart>rend||updateIndexEnd<rstart)
        return st[index];
//NOW if lazy index of here is zero
   else if(rstart==updateIndexStart&&rend==updateIndexEnd){
       st[index]+=(rend-rstart+1)*diff;
       if(rstart!=rend){
           //propagate lazy to children
           lazy[2*index]+=diff;
           lazy[2*index+1]+=diff;
 ...
 /*   if (updateIndexEnd <= rmid)
        //the problem can be solve in left alone
        return updateRangeUtil(rstart, rmid, updateIndexStart,updateIndexEnd, diff, index << 1);
    else if(updateIndexStart>=rmid+1)
        return updateRangeUtil(rmid + 1, rend, updateIndexStart,updateIndexEnd, diff, (index << 1) + 1);//+ is of much presedence that <<
*/
        st[index]=updateRangeUtil(rstart, rmid, updateIndexStart,rmid, diff, index << 1)
                +updateRangeUtil(rmid + 1, rend, rmid+1,updateIndexEnd, diff, (index << 1) + 1);
 return st[index];
}
